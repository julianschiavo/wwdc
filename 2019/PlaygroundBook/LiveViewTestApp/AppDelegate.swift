...
class AppDelegate: LiveViewHost.AppDelegate {
    override func setUpLiveView() -> PlaygroundLiveViewable {
        ...
        var emojiTypes: [Emoji.Category] = [.smile, .smileEyes, .neutral, .neutralEyes, .wink, .sleep]
        
        let manager = EmojizeGameManager(emojiTypes: emojiTypes)
        return manager.viewController
    }
